...
**Real hero/closing/RSVP photos (2026-08)**: the couple supplied ten candidate phone photos in `assets/img/candidates/` and picked three favorites. Cropped/re-encoded with `sips` (no ImageMagick available) to fit each section's full-bleed `.section-bg`: `hero-bg-photo.jpg` (Algarve coastline shot, 1600×900 centered crop — reintroduces a hero background photo, reversing the "no hero photo" decision from the feedback pass above now that a photo the couple actually likes exists), `closing-bg-photo.jpg` (warm backlit indoor selfie, 1536×1150 crop keeping both faces in frame — replaces `closing-bg-placeholder.svg`, deliberately a different mood/setting from the hero per the earlier "must differ from the hero" note), and `rsvp-bg-photo.jpg` (plain-wall winter selfie, 1200×900 crop — replaces `rsvp-bg-placeholder.svg`). The unchosen candidates stay in `assets/img/candidates/` as spares; that folder is untracked and listed in `.gitignore` so it never gets committed.

**RSVP photo swap (2026-08)**: `rsvp-bg-photo.jpg` was swapped for a different candidate — a canyon/cliffside selfie (`IMG-20260813-WA0010.jpg`), cropped to 1200×1250 with extra headroom above both faces. `background-position: 82% 15%` on the `.section-bg` biases the crop toward the top-right of the photo (where the faces sit) instead of the default center, so `background-size: cover` doesn't slice through eyes on short/wide viewports or crop out the right-most face on narrow mobile ones — the same fix already applied to the closing and hero photos after similar feedback. Turned out the real bug was structural, not a positioning one: `.rsvp-section` is `min-height`-only and grows taller than the viewport once the guest-list form renders, and `.section-bg` (`position:absolute; inset:0`) stretches to match that grown box — so the cover-crop kept re-stretching (and re-hiding faces) as the form got taller. Fixed with `background-attachment: fixed` on `.rsvp-section .section-bg`, which pins the cover-sizing to the viewport instead of the element's box, regardless of form height.

**Intro section redesign (2026-08)**: `#intro` ("Nuestra historia") was a centered, photo-less typographic pause; restructured to a two-column photo/text layout — a photo collage on the left (`intro-photo-main.jpg`, a flower-arch mirador shot, plus two smaller stacked photos `intro-photo-2.jpg`/`intro-photo-3.jpg` from hikes) and the story copy on the right, mirroring `#wedding`'s split layout but with sides swapped. The "Nuestra historia" label was promoted from a small `.eyebrow` `<p>` to an `<h2>`, picking up the same `.section h2` sizing every other section title uses (it read noticeably smaller than "La Boda" otherwise).
...
16. **Not yet done, needs the couple's input:** real descriptions for Cóctel/Cena/Fiesta in the schedule (Ceremonia now has a real time), guest list itself, Apps Script deployment + `APPS_SCRIPT_URL` wiring, DNS check for `bodabetinamiguel.dpdns.org`, real floral corner artwork
17. ~~Editorial structure redesign: hero/intro/wedding/date/venue/location/schedule/gallery/information/closing, semantic typography scale, accessible information accordion, hero parallax, timeline line-reveal, `og:image`~~ — done, still needs: gallery photos, a dedicated `og-image` composition, and real copy for the "Alojamiento" / "Niños" information items (currently "por confirmar" placeholders; "Llegada y aparcamiento" now has real copy)
18. ~~Real hero/closing/RSVP background photos, picked from candidate phone photos the couple supplied~~ — done
19. ~~Reception time moved to 17:30 (ceremony itself stays at 18:00, giving guests a 30-minute arrival buffer); added real "Llegada y aparcamiento" copy noting on-site parking for all guests~~ — done
